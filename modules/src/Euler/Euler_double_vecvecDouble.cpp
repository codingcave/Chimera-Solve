...
Euler_double_vecvecDouble::Euler_double_vecvecDouble(
...
{
    //ctor
    _euler = new odeint::euler<vec_vec_real>();
    _dt = dt;
    _time = 0;
    _system = new EulerSystem<double, vec_vec_real>(system);
...
size_t Euler_double_vecvecDouble::getTimeType() const
{
    return chimera::systemtypes::PID_NUMBER;
}

size_t Euler_double_vecvecDouble::getStateType() const
{
...
void * Euler_double_vecvecDouble::currentTime()
{
    return new double(_time);
}

//struct T_VectorDef* RungeKutta::getState()
void * Euler_double_vecvecDouble::currentState()
{
    return new struct chimera::simulation::T_VectorDef({_state->size(), true, false, _state});
}

const double& Euler_double_vecvecDouble::getTime() const
{
    return _time;
}

const vec_vec_real& Euler_double_vecvecDouble::getState() const
{
    return *_state;
}

void Euler_double_vecvecDouble::nextStep()
{
    _euler->do_step( *_system , *_state , _time , _dt );
    _time += _dt;
}

void Euler_double_vecvecDouble::initialize(chimera::vec_t_LuaItem args)
{
    if(args.size() > 1) {
...
void Euler_double_vecvecDouble::start(chimera::vec_t_LuaItem args)
{
...
bool Euler_double_vecvecDouble::finished() const
{
