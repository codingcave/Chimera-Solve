...
chimera::simulation::AbstractInitializer* RandomInitializerModule::getInitializerInstance(chimera::vec_t_LuaItem& parameters) const
{
    ...
    {
        if(parameters.size() > 0 && getChimeraSystem()->getTypeSystem()->getParameterBase(parameters[0].getType()) == chimera::systemtypes::PID_INSTANCE && getChimeraSystem()->getTypeSystem()->getParameterTag(parameters[0].getType()) == randomEntryPoint)
        {
